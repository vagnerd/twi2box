...
<body>
  <div id="main">
   ...
   <form method="post" action="mgr-folders.php">

	<?php
	foreach ($Boxes as $Box) {
   	if ($Box == "inbox" || $Box == "sent" || $Box == "trash") {
		echo "<tr><td><INPUT TYPE=\"checkbox\" NAME=\"folder[]\" VALUE=\"$Box\" DISABLED></td><td>$Box</td></tr>\n"; }
	else {
		$DISABLED="";
		if (CheckMessagesFolder($BoxID,$Box)) { $DISABLED="DISABLED"; }
		echo "<tr><td><INPUT TYPE=\"checkbox\" NAME=\"folder[]\" VALUE=\"$Box\" $DISABLED></td><td>$Box</td></tr>\n"; }
	}
	?>
	</table>
   ...
   <p></p>
   <p>New Folder:</p>
   <p><input type="text" name="newfolder"></p>
   <input type="submit" value="New Folder" name="acao">
