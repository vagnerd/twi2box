...
if ($xMessageID=$_GET["msgid"]) {
   $xMessage=GetMessage($xMessageID);
...
<body>
  <div id="main">
	...
	if ($_POST["tomessage"] == "" && $_POST["message"] == "") {
	   echo "<form action='compose.php' method='post'>";
	   echo "<p>To:</p> <p><input type='text' name='tomessage' value='@$xMessage[1]'></p>";
	   echo "<p><textarea name='message'>$xMessage[2]</textarea></p>";
	   echo "<p><input class='submit' type='submit' value='reply'></p>";
	} else {
	    $Message="".$_POST["tomessage"]." ".$_POST["message"]."";
 	    ComposeSentMessage($Message,$xTwitterAccessToken['oauth_token'],$xTwitterAccessToken['oauth_token_secret']);
	     header("Location: ./");
	  }	
